...
def fix(world: Path, source_world: Path, version: int, get_confirmation: bool) -> FixWorldFlags:
    MK.MKprint("Fixing world data","EMU","CYAN")
    ...
    if not source_world.exists():
        MK.printError("ERROR: Source copy of world does not exist!")
        return flags
    ...
    MK.MKprint("Fixing regions","EMU","CYAN")
    ...
    MK.MKprint("Fixing scoreboard.dat","EMU","CYAN")
    fix_scoreboard_dat(world)

    # Fix maps
    MK.MKprint("Fixing maps","EMU","CYAN")
    fix_maps(world)
...
def fix_region_file(file_path: Path, source_file_path: Path, dimension: str):
    MK.printColoredWithoutNewLine(f" Fixing region ","BRIGHT_YELLOW")
    ...
    for chunk_metadata in cast(list[region.ChunkMetadata], region_file.get_metadata()):
        ...
        if defaults.DEBUG_MODE:
            MK.printColoredWithoutNewLine(f" Fixing chunk ","BRIGHT_MAGENTA")
            ...
            if block_entity["id"].value == "minecraft:command_block":
                ...
                if "LastOutput" in block_entity:
                    try:
                        ...
                        MK.printError(f"ERROR: An error occurred while updating a JSON text component: {block_entity["LastOutput"].value}")
                        utils.log_error()
...
def fix_structure_folder(folder: Path, source_folder: Path, namespace: str):
    for source_file_path in source_folder.glob("**/*.nbt"):
        ...
        MK.printColoredWithoutNewLine(f" Fixing structure ","BRIGHT_GREEN")
