...
def read_commands(world: Path):
    MK.MKprint("Reading command block data","EMU", "CYAN")
...
def read_commands_from_region(world: Path, file_path: Path, list_name: str) -> str | None:
    ...
    MK.printColoredWithoutNewLine(f" Reading ","BRIGHT_YELLOW")
...
def write_commands(world: Path, get_confirmation: bool):
    MK.MKprint("Writing command block data","EMU","CYAN")
    ...
    if not (PROGRAM_PATH / "commands.mcfunction").exists():
        MK.printError("ERROR: commands.mcfunction does not exist!")
        return
...
def update_commands(version: int):
    MK.MKprint("Updating command block data","EMU","CYAN")
    ...
    if not (PROGRAM_PATH / "commands_original.mcfunction").exists():
        MK.printError("ERROR: commands_original.mcfunction does not exist!")
        return
...
def extract_commands(world: Path):
    ...
    if (
        ...
        MK.printError("ERROR: Location is malformed!")
        return
