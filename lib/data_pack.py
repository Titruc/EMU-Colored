...
def update(world: Path, version: int):
    MK.MKprint("Updating data packs","EMU","CYAN")
    ...
    if not source_world.exists():
        MK.printError("ERROR: Source copy of world doesn't exist, prepare it!")
        return
    if not (world / "datapacks").exists():
        MK.printError("ERROR: World has no data packs!")
        return
    if not (source_world / "datapacks").exists():
        MK.printError("ERROR: Source copy of world has no data packs!")
        return
    ...
    for data_pack in (source_world / "datapacks").iterdir():
        ...
        try:
            ...
            MK.printError(f"An error was thrown while updating the data pack: {data_pack.name}")
            utils.log_error()
...
def update_data_pack(pack: Path, source_pack: Path, data_pack: str):
    MK.printColoredWithoutNewLine("Updating ","MAGENTA")
    MK.printColored(f"{data_pack}","DEFAULT")
...
def update_functions(folder: Path, source_folder: Path, namespace: str):
    for source_file_path in source_folder.glob("**/*.mcfunction"):
        ...
        try:
            ...
            MK.printError(f"ERROR: An error occurred while updating the function: {source_file_path.as_posix()}")
            utils.log_error()
...
def update_advancements(folder: Path, source_folder: Path):
    for source_file_path in source_folder.glob("**/*.json"):
        ...
        try:
            ...
            MK.printError(f"ERROR: An error occurred when updating advancement: {source_file_path.as_posix()}")
            utils.log_error()
...
def update_predicates(folder: Path, source_folder: Path):
    for source_file_path in source_folder.glob("**/*.json"):
        ...
        try:
            ...
            MK.printError(f"ERROR: An error occurred when updating predicate: {source_file_path.as_posix()}")
            utils.log_error()
...
def update_loot_tables(folder: Path, source_folder: Path):
    for source_file_path in source_folder.glob("**/*.json"):
        ...
        try:
            ...
            MK.printError(f"ERROR: An error occurred when updating loot table: {source_file_path.as_posix()}")
            utils.log_error()
...
def update_recipes(folder: Path, source_folder: Path):
    for source_file_path in source_folder.glob("**/*.json"):
        ...
        try:
            ...
            MK.printError(f"ERROR: An error occurred when updating recipe: {source_file_path.as_posix()}")
            utils.log_error()
...
def update_item_modifiers(folder: Path, source_folder: Path):
    for source_file_path in source_folder.glob("**/*.json"):
        ...
        try:
            ...
            MK.printError(f"ERROR: An error occurred when updating item modifier: {source_file_path.as_posix()}")
            utils.log_error()
...
def update_tags(folder: Path, source_folder: Path, tag_type: str):
    for source_file_path in source_folder.glob("**/*.json"):
        ...
        try:
            ...
            MK.printError(f"ERROR: An error occurred when updating tag: {source_file_path.as_posix()}")
            utils.log_error()
...
def update_structures(folder: Path, source_folder: Path, namespace: str):
    for source_file_path in source_folder.glob("**/*.nbt"):
        ...
        pack_subdir = source_file_path.as_posix()[len(source_folder.as_posix()) + 1:]
        file_path = folder / pack_subdir
        MK.printColoredWithoutNewLine(f" Fixing structure ","YELLOW")
        MK.printColored(f"{namespace}:{pack_subdir[:-4]}","DEFAULT")
        try:
            structure.update(file_path, source_file_path, pack_version)
        except Exception:
            MK.printError(f"ERROR: An error occurred when updating structure: {source_file_path.as_posix()}")
            utils.log_error()
        


def rename_directories(world: Path, get_conformation: bool):
    MK.MKprint("Renaming data pack directories","EMU","CYAN")
                    ...
                    for folder_pair in DIRECTORY_RENAMES:
                        ...
                        if source_path.exists() and source_path.is_dir():
                            if target_path.exists():
                                MK.printError(f'ERROR: Cannot rename directory {source_path}, target path already exists')
                            else:
...
def extract_stored_functions(world: Path, get_confirmation: bool):
    MK.MKprint("Extracting stored functions","EMU","CYAN")
    ...
    if not (world / "data" / "functions"):
        MK.printError("ERROR: World has no stored functions!")
        return
...
def extract_stored_advancements(world: Path, get_confirmation: bool):
    MK.MKprint("Extracting stored advancements","EMU","CYAN")
    ...
    if not (world / "data" / "advancements"):
        MK.printError("ERROR: World has no stored advancements!")
        return
...
def disable_folder(world: Path, folder: str, get_confirmation: bool):
    MK.printColoredWithoutNewLine("Disabling ","BLUE")
    MK.printColored(f"{folder}","DEFAULT")
...
def fix_disabled_vanilla(world: Path):
    MK.MKprint("Fixing disabled vanilla data pack","EMU","CYAN")
    ...
    if (world / "datapacks" / "vanilla_disabler").exists():
        MK.printError("ERROR: Vanilla disabler already exists!")
        return
...
def unzip_packs(world: Path, get_confirmation: bool):
    MK.MKprint("Unzipping data packs","EMU","CYAN")
    ...
    for file_path in (world / "datapacks").iterdir():
        ...
        if folder.exists() and folder.is_dir():
            MK.printError(f"ERROR: {folder.name} is already unzipped!")
            continue

        # Unzip data pack
        MK.printColoredWithoutNewLine(f"Unzipping ","BRIGHT_CYAN")
        MK.printColored(f"{folder.name}","DEFAULT")
    ...
    if not has_packs:
        MK.printError("ERROR: There are no zipped data packs!")
        return
...
def zip_packs(world: Path, get_confirmation: bool):
    MK.MKprint("Zipping data packs","EMU","CYAN")
    ...
    for folder in (world / "datapacks").iterdir():
        ...
        # Zip folder
        MK.printColoredWithoutNewLine(f"Zipping ","GRAY")
        MK.printColored(f"{folder.name}","DEFAULT")
    ...
    if not has_packs:
        MK.printError("ERROR: There are no unzipped data packs!")
        return
...
def merge_packs(world: Path, name: str):
    MK.MKprint("Merging data packs","EMU","CYAN")
    ...
    if (world / "datapacks" / (name + " Data Pack")).exists():
        MK.printError("ERROR: Output data pack already exists!")
        return
    ...
    if (world / "datapacks" / (name + " Data Pack") / "data" / "minecraft" / "advancement").exists():
        MK.printColored("Advancements in the 'minecraft' namespace were found, consider disabling them","LIGHT_RED")
    if (world / "datapacks" / (name + " Data Pack") / "data" / "minecraft" / "recipe").exists():
        MK.printColored("Recipes in the 'minecraft' namespace were found, consider disabling them","LIGHT_RED")

def merge_pack(pack_a: Path, pack_b: Path, data_pack: str):
    MK.printColoredWithoutNewLine(f"Merging ","GREEN")
    MK.printColored(f"{data_pack}")
