...
def log_error():
    print("")
    MK.printError(f'ERROR:\n{traceback.format_exc()}')
    MK.printError(f'Error logged to: {get_log_path().as_posix()}')
    MK.printColoredWithoutNewLine(f'Please report the issue on the',"DEFAULT")
    MK.printColoredWithoutNewLine('E.M.U. Discord server',"BLUE")
    MK.printColored(f': {defaults.DISCORD_INVITE}',"DEFAULT")
...
def safe_file_read(file_path: Path) -> str:
    ...
    MK.printError(f"Could not read file, returning empty string: {file_path.as_posix()}")
    return ""
...
def safe_file_write(file_path: Path, contents: str):
    ...
    MK.printError(f"Could not write to file: {file_path.as_posix()}")