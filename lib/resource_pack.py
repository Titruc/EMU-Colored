...
def update(pack: Path, version: int):
    MK.MKprint("Updating resource pack","EMU","CYAN")
    ...
    if not og_pack.exists():
        MK.printError("ERROR: Original copy of resource pack does not exist!")
        return
    if not (og_pack / "pack.mcmeta").exists():
        MK.printError("ERROR: Resource pack does not have pack.mcmeta!")
        return
    if not (og_pack / "assets").exists():
        MK.printError("ERROR: Resource pack does not have assets!")
        return

    dirpath, dirnames, filenames = next(os.walk(og_pack / "assets"))
    if len(dirnames) == 0:
        MK.printError("ERROR: Resource pack has no namespaces!")
        return
    ...
    try:
        ...
        MK.printColored(f"An error occurred while updating the resource pack: {pack.as_posix()}","LIGHT_RED")
        utils.log_error()
...
def update_file_names(og_pack: Path, pack: Path):
    MK.MKprint("Updating file names","EMU", "CYAN")
...
def update_file_name(og_namespace: Path, namespace: Path, subdir: str, target: str | dict[str, Any]):
    ...
    if subdir in [
        ...
        MK.printColored(f'WARNING: Edge case file found: {subdir}',"LIGHT_RED")
...
def fix(pack: Path):
    MK.MKprint("Fixing resource pack","EMU","CYAN")

    if not pack.exists():
        MK.printError("ERROR: Resource pack does not exist!")
        return
...
def import_pack(world: Path, pack: Path, get_confirmation: bool):
    MK.MKprint("Importing resource pack","EMU","CYAN")
    ...
    if not world.exists():
        ...
        return
    if not file.exists():
        MK.printError("ERROR: resources.zip does not exist!")
        return
    ...
    try:
        ...
        MK.printColored(f"An error occurred when importing the resource pack","LIGHT_RED")
        utils.log_error()

def export_pack(world: Path, pack: Path, get_confirmation: bool):
    MK.MKprint("Exporting resource pack","EMU", "CYAN")
    ...
    if not world.exists():
        ...
        return
    if not pack.exists():
        MK.printError("ERROR: Resource pack doesn't exist!")
        return
    ...
    try:
        ...
        MK.printColored(f"An error occurred when exporting the resource pack: {pack.as_posix()}","LIGHT_RED")
        utils.log_error()



def purge_vanilla_assets(pack: Path):
    MK.MKprint("Purging vanilla assets from resource pack","EMU", "CYAN")
    ...
    if not vanilla_pack.exists():
        MK.printError("ERROR: Vanilla resource pack does not exist!")
        return
