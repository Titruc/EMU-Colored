...
def apply_breakpoints(world: Path):
    ...
    if not (data_pack_path / "data").exists():
        MK.printError("ERROR: Data pack does not exist!")
        return
    MK.MKprint("Applying breakpoints","EMU","CYAN")

    MK.MKprint("Copying data pack","CYAN")
    ...
    MK.MKprint("Generating function database","EMU","CYAN")
    data_pack_dict = get_function_names(new_data_pack_path / "data")
    
    MK.MKprint("Hooking up function inheritance","EMU","CYAN")
    apply_function_inheritance(data_pack_dict)

    MK.MKprint("Marking functions with breakpoints","EMU","CYAN")
    mark_breakable_functions(data_pack_dict)
    
    MK.MKprint("Applying breakpoints to functions","EMU","CYAN")
    modify_functions(data_pack_dict, new_data_pack_path)

    MK.MKprint("Breakpoints applied","EMU","CYAN")
