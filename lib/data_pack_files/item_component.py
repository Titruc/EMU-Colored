...
def update_path(path_parts: list[str], version: int, issues: list[dict[str, str | int]]) -> list[str]:
    ...
    if path_parts[1] == "AttributeModifiers":
        ...
        if defaults.SEND_WARNINGS:
            MK.printError(f'WARNING: Invalid child of item tag "AttributeModifiers{path_parts[2]}": {path_parts[3]}')
        return ["components", "minecraft:attribute_modifiers", "modifiers", path_parts[2]]
    ...
    if path_parts[1] == "CanDestroy":
        ...
        if defaults.SEND_WARNINGS:
            MK.printError(f'WARNING: Children of item tag {path_parts[1]} are not handled yet in component conversion')
    ...
    if path_parts[1] == "Charged":
        if defaults.SEND_WARNINGS:
            MK.printError(f'WARNING: Item tag "Charged" was removed')
    ...
    if path_parts[1] == "Decorations":
        ...
        if defaults.SEND_WARNINGS:
            MK.printError(f'WARNING: Children of item tag "Decorations" are not handled yet in component conversion')
        return ["components", "minecraft:map_decorations"] + path_parts[2:]

    if path_parts[1] == "display":
        if len(path_parts) == 2:
            if defaults.SEND_WARNINGS:
                MK.printError(f'WARNING: Item tag "display" cannot properly be fetched on its own as a component')
        ...
        if defaults.SEND_WARNINGS:
            MK.printError(f'WARNING: Invalid child of item tag "display": {path_parts[2]}')
        return ["components", "minecraft:custom_name"]
    ...
    if path_parts[1] == "Explosion":
        ...
        if defaults.SEND_WARNINGS:
            MK.printError(f'WARNING: Invalid child of item tag "Explosion": {path_parts[2]}')
        return ["components", "minecraft:firework_explosion"]
    ...
    if path_parts[1] == "Fireworks":
        ...
        if path_parts[2] == "Explosions":
            ...
            if defaults.SEND_WARNINGS:
                MK.printError(f'WARNING: Invalid child of item tag "Fireworks.Explosions{path_parts[3]}": {path_parts[4]}')
            return ["components", "minecraft:fireworks", "explosions", path_parts[3]]
        if path_parts[2] == "Flight":
            return ["components", "minecraft:fireworks", "flight_duration"]
        if defaults.SEND_WARNINGS:
            MK.printError(f'WARNING: Invalid child of item tag "Fireworks": {path_parts[2]}')
        return ["components", "minecraft:fireworks"]
    ...
    if path_parts[1] == "HideFlags":
        if defaults.SEND_WARNINGS:
            MK.printError(f'WARNING: Item tag {path_parts[1]} cannot be converted for NBT paths')
    ...
    if path_parts[1] == "pages":
        if defaults.SEND_WARNINGS:
            MK.printError(f'WARNING: Item tag {path_parts[1]} depends on item context, item is either minecraft:writable_book or minecraft:written_book')
        if len(path_parts) == 2:
    ...
    if path_parts[1] == "SkullOwner":
        ...
        if defaults.SEND_WARNINGS:
            MK.printError(f'WARNING: Invalid child of item tag "SkullOwner": {path_parts[2]}')
        return ["components", "minecraft:profile", "name"]
    ...
    if path_parts[1] == "Trim":
        ...
        if defaults.SEND_WARNINGS:
            MK.printError(f'WARNING: Invalid child of item tag "Trim": {path_parts[2]}')
        return ["components", "minecraft:trim"]
