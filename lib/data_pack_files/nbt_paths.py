...
def extract_nbt_from_path(nbt, path_parts: list[str]):
    ...
    if path_parts[0].startswith("["):
        ...
        if part_contents.startswith("{") and defaults.SEND_WARNINGS:
            MK.printError(f'WARNING: NBT list index used in NBT extraction by path is not yet supported!')
            return
...
def edge_case(path_parts: list[str], case_type: str, source: str, issues: list[dict[str, str | int]]) -> list[str]:
    ...
    if case_type == "shot_from_crossbow":
        MK.printError(f'WARNING: Entity tag "ShotFromCrossbow" used in an NBT path and was converted to "weapon.id", check that its use is valid')
        return ["weapon", "id"]
