...
def program():
    # Show title
    print("")
    MK.printColored("Easy Map Updater", "CYAN")
    MK.printColoredWithoutNewLine("- By ","DEFAULT")
    MK.printColoredWithoutNewLine("Dominexis","BLUE")
    MK.printColoredWithoutNewLine(", and ","DEFAULT")
    MK.printColoredWithoutNewLine("StickyPiston ","GREEN")
    MK.printColored("Hosting","DEFAULT")
    MK.printColoredWithoutNewLine("- Colored version by ","DEFAULT")
    MK.printColoredWithoutNewLine("Titruc","YELLOW")
    ...
    while True:
        ...
        if action not in actions:
            MK.printError("ERROR: Must be a valid action!")
            continue
        print("")
...
def list_actions():
    MK.MKprint("Actions:", "EMU", "CYAN")
    for action in actions:
        if action == Action.VERSION.value:
            print("")
        if actions[action]["show"]:
            MK.printColoredWithoutNewLine(str(action), "DEFAULT")
            MK.printColored(f"  {' '*max(21-len(action), 0)}{actions[action]['name']}", "GRAY")
...
def action_update(): # Needs confirmation
    MK.MKprint("Updating map","EMU","CYAN")
    ...
    if update_progress["stage"] == 400:
        ...
        if confirm not in ["y", "Y"]:
            MK.printColored("Updated canceled","LIGHT_RED")
            return
        next_update_progress_section()
    ...
    print("")
    MK.MKprint("Map updated","EMU", "CYAN")
...
def action_clear_command_helper(): # Needs confirmation
    ...
    MK.MKprint("Clearing command helper")

    if not data_pack_path.exists():
        MK.printError("ERROR: Command helper does not exist!","EMU","CYAN")
        return
...
def action_prepare_original_copy_world(manual: bool = True): # Needs confirmation
    MK.MKprint("Preparing original world copy","EMU","CYAN")
...
def action_toggle_debug_mode():
    if defaults.DEBUG_MODE:
        ...
        MK.printColored("Disabled debug mode","LIGHT_RED")
    else:
        defaults.DEBUG_MODE = True
        MK.printColored("Enabled debug mode","LIGHT_RED")
...
def retrieve_file_path(message: str) -> Path | None:
    while True:
        ...
        if not file_path.exists():
            MK.printError("ERROR: File does not exist!")
            continue
        return file_path
...
def retrieve_json_file_contents(message: str) -> Any:
    while True:
        ...
        if not load_bool:
            MK.printError("ERROR: File could not be loaded!")
            continue
        return contents
